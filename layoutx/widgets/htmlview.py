# ========================================== #
# MIT License                                #
# Copyright (c) 2020 Palash Bauri            #
# Copyright (c) 2018 paolo-gurisatti         #
...
import webbrowser
import tkinter as tk
from tkinter import font, ttk, scrolledtext, font
...
class Defs():
    DEFAULT_TEXT_FONT_FAMILY = ("Roboto", "Segoe ui", "Calibri", "Helvetica", "TkTextFont")
    FONT_SIZE = 12
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag in HTML.STYLE_TAGS:
            ...
            if attrs[HTML.Attrs.SRC].startswith(("https://" , "ftp://" , "http://")):
                raise NotImplemented("External Links not supported")
            
            if not "base64" in attrs[HTML.Attrs.SRC]:
                raise NotImplemented("Only base64 encoding supported")
            
            srcContent = re.search(pattern=r'data:([^;]*?);([^,]*?),(.*)', flags=re.I, string=attrs[HTML.Attrs.SRC])
            image = Image.open(BytesIO(b64decode(srcContent.group(3))))
...
class _HTMLScrolledText(_ScrolledText):
    #----------------------------------------------------------------------------------------------
    """
...
class _HTMLText(_HTMLScrolledText):
    #----------------------------------------------------------------------------------------------
    """
    HTML text widget
    """
...
class HTMLScrolledText(Widget):
    def __init__(self, master, **kwargs):
        super().__init__(tk=_HTMLScrolledText(master=master), **kwargs)

    def on_init(self):
        self.connect_to_prop("html", self.on_changed_html)

    def on_changed_html(self, value):
        self._node.widget._tk.set_html(value)

class HTMLText(Widget):
    def __init__(self, master, **kwargs):
        super().__init__(tk=_HTMLText(master=master), **kwargs)
...
class HTMLLabel(Widget):
    def __init__(self, master, **kwargs):
        super().__init__(tk=_HTMLLabel(master=master), **kwargs)
